...
On pourrait croire que la réversibilité nous protège : rien ne se perd, donc rien ne se crée ? Hélas, non — et il vaut la peine de le voir de ses yeux. Prenons l'état initial le plus innocent : $`u(-1) = 0`$ partout, et $`u(0) = \delta_0`$ (un seul point à $`1`$). Sur la chaîne avec $`c=1`$, les premières photographies sont :
```math
\begin{array}{c|rrrrrrr|c}
 & u_{-3} & u_{-2} & u_{-1} & u_0 & u_1 & u_2 & u_3
 & S(t)=\sum_n u_n(t) \\ \hline
t=-1 & 0 & 0 & 0 & 0 & 0 & 0 & 0 & 0 \\
t=0  & 0 & 0 & 0 & 1 & 0 & 0 & 0 & 1 \\
t=1  & 0 & 0 & 1 & 0 & 1 & 0 & 0 & 2 \\
t=2  & 0 & 1 & 0 & 1 & 0 & 1 & 0 & 3 \\
...
\end{array}
```
Le symbole $`\delta_0`$ devient concret : c'est simplement la ligne $`t=0`$, nulle partout sauf au sommet $`0`$. Dès les lignes suivantes, le signal se propage et la somme augmente. Pour comprendre pourquoi ce phénomène ne dépend pas de cet exemple particulier, suivons la somme totale $`S(t) = \sum_v u_v(t)`$. En sommant la règle sur tous les nœuds, le terme de Laplacien s'évanouit (chaque arête donne $`+`$ d'un côté ce qu'elle donne $`-`$ de l'autre : $`\mathbf{1}^\top L = 0`$), et il reste
```math
S(t+1) = 2S(t) - S(t-1).
```
...
**Que mesurent exactement ces deux termes ?** Le terme cinétique compare les deux photographies : il est grand lorsque les valeurs changent vite. Le terme de forme compare, sur chaque arête, la différence d'aujourd'hui avec celle d'hier. Il mesure donc une tension spatiale qui persiste à travers l'intervalle de temps. Si le champ varie lentement, $`u(t-1)\simeq u(t)`$, et il devient presque l'énergie de Dirichlet ordinaire $`\tfrac{c^2}{2}u(t)^\top Lu(t)`$ : l'énergie d'une corde ou d'un réseau de ressorts déformé. La présence de deux temps n'est pas une force mystérieuse qui « pousse » le futur ; elle vient de ce que l'énergie exacte de notre schéma discret est naturellement attachée à l'intervalle entre deux photographies. C'est toujours la règle d'évolution qui produit le mouvement.

L'image physique tient en une phrase : **la forme rend le Laplacien non nul, le Laplacien produit une accélération, et la déformation se convertit alors en mouvement**. Attention toutefois : dans une évolution très rapide, le terme croisé $`u(t)^\top Lu(t-1)`$ peut être négatif. Les deux morceaux ne sont donc pas toujours, séparément, deux réserves positives ; c'est leur somme exacte $`E`$ qui constitue le compte fiable.

### Voir l'échange sur une seule arête

Prenons le graphe le plus petit possible, deux sommets $`A-B`$, et choisissons $`c^2=\tfrac12`$. Notons
```math
q=\begin{pmatrix}1\\-1\end{pmatrix},
\qquad Lq=2q,
```
la forme où l'arête est tendue. Si $`u(t)=a_tq`$, la règle du champ se réduit à
```math
a_{t+1}=a_t-a_{t-1}.
```
Partons d'une déformation immobile, $`a_{-1}=a_0=1`$. On obtient successivement $`a_1=0`$, $`a_2=-1`$, puis $`a_3=-1`$. Comme $`\|q\|^2=2`$ et $`q^\top Lq=4`$, les deux termes d'énergie deviennent ici
```math
K(t)=(a_t-a_{t-1})^2,
\qquad
V(t)=a_ta_{t-1}.
```
Le calcul entier tient dans ce tableau :
```math
\begin{array}{c|rr|c|ccc}
t & a_{t-1} & a_t & u(t) & K(t) & V(t) & E(t) \\ \hline
0 &  1 &  1 & ( 1,-1) & 0 & 1 & 1 \\
1 &  1 &  0 & ( 0, 0) & 1 & 0 & 1 \\
2 &  0 & -1 & (-1, 1) & 1 & 0 & 1 \\
3 & -1 & -1 & (-1, 1) & 0 & 1 & 1
\end{array}
```
À $`t=0`$, l'arête est déformée mais ne bouge pas : toute l'énergie est de forme. À $`t=1`$, elle passe par la configuration plate, mais la différence entre les deux photographies est maximale : toute l'énergie est cinétique. À $`t=2`$, la déformation opposée est déjà visible, mais la photographie précédente était plate : le terme de forme, qui exige une tension commune aux deux images, vaut donc encore zéro. Il ne redevient égal à $`1`$ qu'à $`t=3`$, lorsque la nouvelle déformation persiste. L'échange semble brutal parce que notre horloge avance par grands pas ; avec un pas de temps plus fin, on retrouverait le va-et-vient progressif d'un ressort ou d'un pendule. Dans tous les cas, $`K`$ et $`V`$ changent tandis que $`E=K+V=1`$ ne bronche pas.
...
**Ce que la conservation garantit — et à quelle condition.** Il faut être précis sur ce que notre théorème achète. Une quantité conservée n'interdit l'explosion que si elle *contrôle effectivement la taille* du champ. Une somme de carrés peut jouer ce rôle si elle ne possède pas de direction non nulle où tous ses carrés s'annulent. La conservation devient alors une **laisse**. Mais une forme seulement semi-positive peut laisser échapper certaines directions, et une forme prenant des valeurs négatives peut permettre à un terme de croître vers $`+\infty`$ pendant qu'un autre plonge vers $`-\infty`$. Être conservé ne suffit donc pas : le bilan doit aussi voir les mouvements que l'on veut borner.
...
```
Tout devient lisible. Le second terme est une rugosité (chapitre I, §5) : il est toujours positif ou nul. Le premier l'est à condition que la matrice $`I-\tfrac{c^2}{4}L`$ soit positive, c'est-à-dire que $`\tfrac{c^2}{4}d^\top Ld`$ ne dépasse jamais $`\|d\|^2`$. Or il existe un nombre attaché au graphe qui mesure précisément le pire facteur d'amplification de $`L`$ : sa plus grande valeur propre $`\lambda_{\max}`$. La condition de non-négativité est
```math
c^2\lambda_{\max}\leq4
\quad\Longleftrightarrow\quad
E\geq0.
```
Mais le cas d'égalité demande encore de la prudence. Si $`c^2\lambda_{\max}=4`$, le premier carré s'annule dans la direction propre la plus rugueuse : l'énergie ne voit plus certains mouvements critiques, qui peuvent croître linéairement en alternant de signe. Pour que tous les modes non constants soient des oscillations bornées, il faut le seuil **strict**
```math
c^2\lambda_{\max}<4.
```
Le mode constant reste un cas à part : aucune énergie de forme ne peut borner sa hauteur absolue, et nous savons déjà qu'il peut dériver linéairement sans créer la moindre tension.

Et si l'on viole le seuil ? La conservation tient toujours — le théorème du §5 est inconditionnel — mais la laisse est cassée, et l'univers en profite. Prenons le graphe le plus petit qui soit : deux nœuds, une arête ($`\lambda_{\max}=2`$), et $`c^2=3`$, en infraction puisque $`3\times2>4`$. Écrivons
```math
u(t)=a_t\begin{pmatrix}1\\-1\end{pmatrix}.
```
Comme $`L(1,-1)^\top=2(1,-1)^\top`$, la règle devient
```math
a_{t+1}=-4a_t-a_{t-1}.
```
Partons d'un souffle : $`a_{-1}=0`$ et $`a_0=0{,}01`$. Dans ce mode,
```math
K(t)=(a_t-a_{t-1})^2,
\qquad
V(t)=6a_ta_{t-1}.
```
Le tableau montre simultanément l'explosion du champ et la compensation des deux termes :
```math
\begin{array}{c|r|r|r|r}
t & a_t & K(t) & V(t) & E(t)=K(t)+V(t) \\ \hline
0  & 0{,}01        & 0{,}0001              & 0                       & 0{,}0001 \\
1  & -0{,}04       & 0{,}0025              & -0{,}0024               & 0{,}0001 \\
2  & 0{,}15        & 0{,}0361              & -0{,}0360               & 0{,}0001 \\
5  & -7{,}80       & 97{,}8121             & -97{,}8120              & 0{,}0001 \\
8  & 405{,}45      & 264\,288{,}5281       & -264\,288{,}5280       & 0{,}0001 \\
11 & -21\,075{,}60 & 714\,107\,505{,}3841 & -714\,107\,505{,}3840 & 0{,}0001
\end{array}
```
Au pas $`11`$, l'amplitude a été multipliée par plus de deux millions. Pourtant, à chaque ligne, le terme cinétique positif et le terme croisé négatif se compensent jusqu'à laisser le même minuscule $`0{,}0001`$. L'univers ne *triche* pas — le bilan est scrupuleusement tenu — mais il *diverge* quand même. Moralité : il ne suffit pas qu'une grandeur soit conservée ; il faut qu'elle soit conservée **et** qu'elle borne.

Voilà qui est intrigant à double titre. D'abord, notre univers n'est digne de confiance que si sa vitesse de propagation $`c`$ reste strictement sous un plafond fixé par la *structure du graphe* : les mondes trop rapides pour leur géométrie sont instables, et le monde exactement critique conserve encore une direction de croissance linéaire. (Un univers qui impose une vitesse limite, notée $`c`$ de surcroît… toute ressemblance avec une célèbre constante de la physique serait, bien entendu, purement fortuite. Soyons honnêtes : notre plafond est un seuil de *stabilité*, pas un principe de relativité, et l'analogie ne doit pas être poussée trop loin. Mais elle n'est pas creuse non plus : la localité impose déjà à toute information de progresser d'au plus une arête par pas de temps — notre monde possède un authentique cône causal — et la condition $`c^2\lambda_{\max}<4`$ dit que la dynamique doit rester compatible avec ce que le tissu discret peut soutenir sans instabilité. Un monde où l'on force l'allure ne viole pas ses lois : il devient instable.) Ensuite, ce plafond fait intervenir un objet d'apparence purement algébrique — la plus grande valeur propre de $`L`$ — surgi ici sans qu'on l'invite, dans une question de pure physique. Nous ne savons pas encore ce que *sont* les vecteurs propres de $`L`$, ni pourquoi le pire d'entre eux gouverne la stabilité du monde entier. C'est précisément ce que le chapitre suivant va élucider — et la réponse dépassera largement cette question de seuil.
...
> **Chapitre suivant — V. Quelles formes restent fixes dans l'espace ?**
