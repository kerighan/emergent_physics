...
Évaluons maintenant $`P`$ sur une onde progressive pure, $`u_n(t) = \cos(kn - \omega t)`$, construite à partir des modes de Fourier du chapitre VI. Le calcul (identités produit-somme, les termes oscillants s'annulant sur l'anneau) donne une formule fermée :
```math
