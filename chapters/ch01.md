...
Donnons-nous la seule chose qu'un point puisse porter : un nombre. Sur chaque sommet $`v`$ du graphe, nous écrivons une valeur $`u_v`$. Nous ne décidons pas encore de l'ensemble de *tous* les nombres autorisés : les entiers et les fractions suffiront à nos premières expériences, et il serait contraire à notre principe de parcimonie d'importer dès maintenant un domaine numérique plus riche. Nous l'agrandirons seulement lorsqu'un calcul l'exigera.

La donnée de toutes les valeurs portées par les sommets sera notée
```math
u=(u_v)_{v\in V},
```
et nous l'appellerons un **champ**. C'est notre unique matière première : une hauteur, une amplitude, une intensité — peu importe le nom — posée sur chaque point de l'univers.
...
**$`L`$ est symétrique.** Puisque $`A`$ est symétrique ($`v\sim w \iff w \sim v`$) et $`D`$ diagonale, $`L=D-A`$ est symétrique : échanger les deux indices ne change aucun coefficient. Nous n'en tirerons pas encore de théorème spectral, car cela demanderait de préciser un domaine numérique plus riche que celui dont nous avons eu besoin jusqu'ici. Au chapitre V, la dynamique elle-même nous donnera une raison d'effectuer cet élargissement — comme outil de description, non comme nouvel axiome sur la substance de l'univers.
