...
Le nombre $`k`$ indique de combien la phase augmente lorsqu'on avance d'une arête. Pour que le dessin se referme après $`N`$ arêtes, il faut
...
Il ne s'agit donc pas d'introduire arbitrairement toutes les valeurs réelles de $`k`$ : sur l'anneau fini, nous avons d'abord une **liste finie** de motifs. Les fonctions trigonométriques sont seulement une écriture commode de cette liste.
...
Attention au comptage : $`k`$ et $`-k`$ ne donnent pas quatre profils indépendants, car
...
Pour un $`k`$ non exceptionnel, le cosinus et le sinus ont la même valeur propre. Leurs amplitudes temporelles obéissent donc à la même récurrence. On peut les faire évoluer avec un quart de cycle de décalage :
...
En remplaçant $`\lambda(k)`$ par $`4\sin^2(k/2)`$, il vient
...
La formule avec les carrés est la formule complète. Si nous choisissons la branche
...
Les petites oscillations se déplacent à la vitesse de phase $`\omega_0/k_0`$. L'enveloppe qui les regroupe se déplace, elle, à la vitesse
...
Sur un anneau fini, ce **rapport de différences** est la quantité directement définie : les $`k`$ disponibles sont espacés. Si l'anneau est assez grand et les deux valeurs assez proches, on résume ce rapport par la pente d'une courbe interpolant la liste :

```math
v_g=\frac{d\omega}{dk}.
```

La dérivée n'ajoute donc aucune nouvelle loi à l'univers discret ; c'est un outil pratique pour lire rapidement une liste très fournie.
